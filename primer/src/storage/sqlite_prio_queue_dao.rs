    ...
    fn insert(&mut self, items: &Vec<(i64, i64)>) -> Result<(), Error> {
        ...
        self.connection
            .execute(
                format!(
                    // language=sqlite
                    "INSERT INTO {table} (priority, value) VALUES {values}",
                    table = self.table,
                    values = items
                        .iter()
...
mod tests {
    ...
    fn insert_multiple_one_by_one() {
        ...
        for i in 0..5 {
            dao.insert(&vec![
                vec![(900, 900), (100, 200), (500, 600), (300, 400), (700, 800)][i],
            ])
            .expect("dao should accept inserts");
    ...
    #[test]
    fn retrieve_multiple_one_by_one() {
        let mut dao = get_dao();
        dao.insert(&vec![
            (900, 900),
            (100, 200),
        ...
        for i in 0..5 {
            ...
            assert_eq!(
                *data.first().expect("first() should return one"),
                vec![(100, 200), (300, 400), (500, 600), (700, 800), (900, 900)][i]
            );
            if i < 4 {
                assert!(!dao.is_empty());
                assert!(dao.is_not_empty());
